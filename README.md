...
- **客户端**(`dsh-cli`,纯 Node 零依赖):逐 token 流式输出、竖排菜单、会话切换、模式/模型/权限切换、审批 y/n。

## 特性

| 能力 | 说明 |
| --- | --- |
| 流式输出 | 回答逐 token 流式显示;思考内容默认折叠(`/think` 查看) |
...
/mode  /model  /permission  /think  /transcript  /cancel  /clear  /exit
```
...
```bash
git clone <repo> && cd dsh-cli-mode
npm run check         # 语法检查
npm test              # 冒烟测试
```
...
(实现:提取 `apply` 与 `inject` 的文本源,写入 `plugin-source.txt`。)
...
GitHub Actions 会在 `v*` tag 上自动发布到 npm(见 `.github/workflows/publish.yml`)。
